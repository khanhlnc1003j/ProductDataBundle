...
namespace Starfruit\ProductDataBundle\Ecommerce;
    ...
    public function addCondition(AbstractFilterDefinitionType $filterDefinition, ProductListInterface $productList, $currentFilter, $params, $isPrecondition = false)
    {
        ...
        if(is_array($value)){
            ...
            for ($i=0; $i < count($value) ; $i++) { 
                if($i == 0){
                    $str .= $filterDefinition->getField() . ' LIKE "%,'.$value[$i].',%"' ;
                }else{
                    $str .= " OR ".$filterDefinition->getField() . ' LIKE "%,'.$value[$i].',%"' ;
                }
            }
            if($str){
                $productList->addCondition($str);
            }
        }else if (!empty($value)) {
            $value = '%,' . trim($value) . ',%';
